...
export default function DashboardLayout({ children, user }: DashboardLayoutProps) {
  ...
  const brandColor = user.role === 'superadmin' 
    ? getDefaultBrandColor() 
    : getOrganizationBrandColor(organization)
  
  const appName = user.role === 'superadmin' 
    ? getAppName() 
          ...
          <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
            {navigation.map((item) => {
              ...
              return (
                ...
                >
                  <span className={`mr-3 ${active ? 'text-white' : 'text-gray-400'}`}>{item.icon}</span>
                  {item.name}
                </Link>
              )
            })}
